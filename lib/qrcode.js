...
GridSampler.checkAndNudgePoints=function( image,  points)
		{
			...
			var height = qrcode.imagedata.height;
...
GridSampler.sampleGrid3=function( image,  dimension,  transform)
		{
			...
			for (var y = 0; y < dimension; y++)
			{
				...
				try
				{
					for (var x = 0; x < max; x += 2)
					{
						var xpoint = (Math.floor( points[x]) * 4) + (Math.floor( points[x + 1]) * qrcode.imagedata.width * 4);
                        var bit = image[Math.floor( points[x])+ qrcode.imagedata.width* Math.floor( points[x + 1])];
	...
	this.sizeOfBlackWhiteBlackRun=function( fromX,  fromY,  toX,  toY)
		{
			...
			for (var x = fromX, y = fromY; x != toX; x += xstep)
			{
				...
				if (state == 1)
				{
					...
					if (this.image[realX + realY*qrcode.imagedata.width])
					{
						state++;
					}
				}
	...
	this.sizeOfBlackWhiteBlackRunBothWays=function( fromX,  fromY,  toX,  toY)
		{
			...
			else if (otherToX >= qrcode.imagedata.width)
			{
				scale =  (qrcode.imagedata.width - 1 - fromX) /  (otherToX - fromX);
				otherToX = qrcode.imagedata.width - 1;
			}
			...
			else if (otherToY >= qrcode.imagedata.height)
			{
				scale =  (qrcode.imagedata.height - 1 - fromY) /  (otherToY - fromY);
				otherToY = qrcode.imagedata.height - 1;
			}
	...
	this.findAlignmentInRegion=function( overallEstModuleSize,  estAlignmentX,  estAlignmentY,  allowanceFactor)
		{
			...
			var alignmentAreaRightX = Math.min(qrcode.imagedata.width - 1, estAlignmentX + allowance);
			...
			var alignmentAreaBottomY = Math.min(qrcode.imagedata.height - 1, estAlignmentY + allowance);
...
qrcode.imagedata = {};
qrcode.imagedata.width = 0;
qrcode.imagedata.height = 0;
...
qrcode.process = function(im, cb){
  ...
  var grayscale = qrcode.grayscale();
	var image = qrcode.grayScaleToBitmap(grayscale);
  ...
  //console.log(detector);
	...
	/*for (var y = 0; y < qRCodeMatrix.bits.Height; y++)
	{
		...
		{
			var point = (x * 4*2) + (y*2 * qrcode.imagedata.width * 4);
	...
	cb(qrcode.decode_utf8(str))
}

qrcode.getPixel = function(x,y){
	if (qrcode.imagedata.width < x) {
		throw "point error";
	}
	if (qrcode.imagedata.height < y) {
		throw "point error";
	}
	point = (x * 4) + (y * qrcode.imagedata.width * 4);
	p = (qrcode.imagedata.data[point]*33 + qrcode.imagedata.data[point + 1]*34 + qrcode.imagedata.data[point + 2]*33)/100;
	return p;
}

qrcode.binarize = function(th){
	var ret = new Array(qrcode.imagedata.width*qrcode.imagedata.height);
	for (var y = 0; y < qrcode.imagedata.height; y++)
	{
		for (var x = 0; x < qrcode.imagedata.width; x++)
		{
			var gray = qrcode.getPixel(x, y);

			ret[x+y*qrcode.imagedata.width] = gray<=th?true:false;
		}
	}
	return ret;
}
...
qrcode.getMiddleBrightnessPerArea=function(image)
{
	...
	var areaWidth = Math.floor(qrcode.imagedata.width / numSqrtArea);
	var areaHeight = Math.floor(qrcode.imagedata.height / numSqrtArea);
	...
	for (var ay = 0; ay < numSqrtArea; ay++)
	{
		for (var ax = 0; ax < numSqrtArea; ax++)
		{
			...
			for (var dy = 0; dy < areaHeight; dy++)
			{
				...
				{
					var target = image[areaWidth * ax + dx+(areaHeight * ay + dy)*qrcode.imagedata.width];
...
qrcode.grayScaleToBitmap=function(grayScale)
{
	...
	var areaWidth = Math.floor(qrcode.imagedata.width / sqrtNumArea);
	var areaHeight = Math.floor(qrcode.imagedata.height / sqrtNumArea);
	var bitmap = new Array(qrcode.imagedata.height*qrcode.imagedata.width);
	...
	for (var ay = 0; ay < sqrtNumArea; ay++)
	{
		for (var ax = 0; ax < sqrtNumArea; ax++)
		{
			for (var dy = 0; dy < areaHeight; dy++)
			{
				for (var dx = 0; dx < areaWidth; dx++)
				{
					bitmap[areaWidth * ax + dx+ (areaHeight * ay + dy)*qrcode.imagedata.width] = (grayScale[areaWidth * ax + dx+ (areaHeight * ay + dy)*qrcode.imagedata.width] < middle[ax][ay])?true:false;
				}
			}
		}
	}
  ...
  var ret = new Array(qrcode.imagedata.width*qrcode.imagedata.height);
	for (var y = 0; y < qrcode.imagedata.height; y++)
	{
		for (var x = 0; x < qrcode.imagedata.width; x++)
		{
			...
			ret[x+y*qrcode.imagedata.width] = gray;
		}
	}
...
function FinderPatternFinder()
{
	...
	this.crossCheckVertical=function( startI,  centerJ,  maxCount,  originalStateCountTotal)
		{
			...
			var maxI = qrcode.imagedata.height;
			var stateCount = this.CrossCheckStateCount;

			// Start counting up from center
			var i = startI;
			while (i >= 0 && image[centerJ + i*qrcode.imagedata.width])
			{
				stateCount[2]++;
				i--;
			}
			...
			while (i >= 0 && !image[centerJ +i*qrcode.imagedata.width] && stateCount[1] <= maxCount)
			{
				stateCount[1]++;
				i--;
			}
			...
			while (i >= 0 && image[centerJ + i*qrcode.imagedata.width] && stateCount[0] <= maxCount)
			{
				stateCount[0]++;
				i--;
			}
			...
			i = startI + 1;
			while (i < maxI && image[centerJ +i*qrcode.imagedata.width])
			{
				stateCount[2]++;
				i++;
			}
			...
			while (i < maxI && !image[centerJ + i*qrcode.imagedata.width] && stateCount[3] < maxCount)
			{
				stateCount[3]++;
				i++;
			}
			...
			while (i < maxI && image[centerJ + i*qrcode.imagedata.width] && stateCount[4] < maxCount)
			{
				stateCount[4]++;
				i++;
			}
	...
	this.crossCheckHorizontal=function( startJ,  centerI,  maxCount, originalStateCountTotal)
		{
			...
			var maxJ = qrcode.imagedata.width;
			var stateCount = this.CrossCheckStateCount;

			var j = startJ;
			while (j >= 0 && image[j+ centerI*qrcode.imagedata.width])
			{
				stateCount[2]++;
				j--;
			}
			...
			while (j >= 0 && !image[j+ centerI*qrcode.imagedata.width] && stateCount[1] <= maxCount)
			{
				stateCount[1]++;
				j--;
			}
			...
			while (j >= 0 && image[j+ centerI*qrcode.imagedata.width] && stateCount[0] <= maxCount)
			{
				stateCount[0]++;
				j--;
			}
			...
			j = startJ + 1;
			while (j < maxJ && image[j+ centerI*qrcode.imagedata.width])
			{
				stateCount[2]++;
				j++;
			}
			...
			while (j < maxJ && !image[j+ centerI*qrcode.imagedata.width] && stateCount[3] < maxCount)
			{
				stateCount[3]++;
				j++;
			}
			...
			while (j < maxJ && image[j+ centerI*qrcode.imagedata.width] && stateCount[4] < maxCount)
			{
				stateCount[4]++;
				j++;
			}
	...
	this.findFinderPattern = function(image){
		...
		for (var i = iSkip - 1; i < maxI && !done; i += iSkip)
		{
			...
			{
				if (image[j+i*qrcode.imagedata.width] )
				{
					// Black pixel
					if ((currentState & 1) == 1)
					{
						...
						if (currentState == 4)
						{
							...
							if (this.foundPatternCross(stateCount))
							{
								...
								else
								{
									...
									while (j < maxJ && !image[j + i*qrcode.imagedata.width]);
...
function AlignmentPatternFinder( image,  startX,  startY,  width,  height,  moduleSize,  resultPointCallback)
{
	...
	this.crossCheckVertical=function( startI,  centerJ,  maxCount,  originalStateCountTotal)
		{
			...
			// Now also count down from center
			i = startI + 1;
			while (i < maxI && image[centerJ + i*qrcode.imagedata.width] && stateCount[1] <= maxCount)
			{
				stateCount[1]++;
				i++;
			}
			...
			while (i < maxI && !image[centerJ + i*qrcode.imagedata.width] && stateCount[2] <= maxCount)
			{
				stateCount[2]++;
				i++;
			}
	...
	this.find = function()
	{
			...
			for (var iGen = 0; iGen < height; iGen++)
			{
				...
				while (j < maxJ && !image[j + qrcode.imagedata.width* i])
				{
					j++;
				}
				var currentState = 0;
